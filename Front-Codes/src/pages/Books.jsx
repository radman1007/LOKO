...
const Books = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [booksRes, classesRes, coinsRes] = await Promise.allSettled([ bookService.getMyBooks(), bookService.getMyClasses(), clubService.getCoins() ]);
        if (!mounted) return;
  ...
  const handleBookClick = (book) => { const gameCount = user?.isGuest ? (book.games?.length || 0) : (book.game_count || 0); if (gameCount > 0) { handleClick(`/book/${book.id}`, book.id); } else { setToast('هنوز هیچ بازی‌ای برای این کتاب ثبت نشده است.'); } };
  const coverFor = (book, index) => { if (book.cover_url && /^https?:\/\//.test(book.cover_url)) return book.cover_url; return FALLBACK_COVERS[index % FALLBACK_COVERS.length]; };
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg, fontFamily: "'Shoor', 'Shoor Rounded', sans-serif", direction: 'rtl', paddingBottom: '80px' }}>
      ...
      <Header 
        ...
        colors={colors} 
        rightAction={
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: colors.coin + '22', padding: '8px 14px', borderRadius: '999px' }}>
            <span style={{ fontSize: '16px' }}>🪙</span>
            <span style={{ fontWeight: '700', color: colors.text, fontSize: '14px' }}>{coins}</span>
          </div>
        }
      />
      ...
      <div style={{ maxWidth: isDesktop ? '1200px' : '100%', margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 24px' }}>
        {classes.length > 0 && (
          <div style={{ marginBottom: '20px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {classes.map((c) => (<span key={c.id} style={{ background: colors.primary + '18', color: colors.primary, padding: '6px 14px', borderRadius: '999px', fontSize: '13px', fontWeight: '600' }}>{c.name}</span>))}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', color: colors.text, padding: '60px 20px' }}><p style={{ fontSize: '15px' }}>هنوز کتابی برای تو ثبت نشده است.</p></div>
        ) : (
              ...
              <div key={book.id} onClick={() => handleBookClick(book)} style={{ cursor: 'pointer', transition: 'transform 0.08s linear', transform: pressedItem === book.id ? 'scale(0.97)' : 'scale(1)' }}>
                <div style={{ background: colors.cardBg, borderRadius: '20px', padding: '16px', textAlign: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                  <div style={{ width: '100%', aspectRatio: '2/3', borderRadius: '16px', overflow: 'hidden', marginBottom: '12px', backgroundColor: colors.primary + '20' }}>
                    <img src={coverFor(book, index)} alt={book.title} onError={(e) => { e.target.src = FALLBACK_COVERS[index % FALLBACK_COVERS.length]; }} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <p style={{ fontSize: '14px', fontWeight: '600', color: colors.text }}>{book.title}</p>
                  {(() => { const gc = user?.isGuest ? (book.games?.length || 0) : (book.game_count || 0); return gc > 0 ? (<p style={{ fontSize: '12px', color: colors.primary, marginTop: '4px', fontWeight: '600' }}>🎮 {gc} بازی</p>) : (<p style={{ fontSize: '12px', color: '#B0BEC5', marginTop: '4px', fontWeight: '600' }}>بدون بازی</p>); })()}
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, right: 0, left: 0, background: colors.navBg, boxShadow: '0 -2px 20px rgba(0,0,0,0.08)', borderRadius: '28px 28px 0 0', padding: isMobile ? '10px 12px' : '12px 20px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 100 }}>
        {navItems.map((item) => { const IconComponent = item.icon; return (<div key={item.id} onClick={() => handleClick(item.path, `nav_${item.id}`)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer', transition: 'transform 0.05s linear' }}><IconComponent size={isMobile ? 20 : 22} color="#999" /><span style={{ fontSize: isMobile ? '9px' : '10px', color: '#999', fontWeight: '400' }}>{item.name}</span></div>); })}
      </div>
    </div>
  );
};
