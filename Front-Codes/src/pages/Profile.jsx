...
const Profile = () => {
  ...
  const loadTickets = async () => { setTicketsLoading(true); try { const res = await apiClient.get('/tickets'); if (res.data.success && Array.isArray(res.data.data)) setTickets(res.data.data); else setTickets([]); } catch (error) { setTickets([]); } finally { setTicketsLoading(false); } };
  ...
  const handleSubmitTicket = async () => { if (!ticketSubject.trim() || !ticketMessage.trim()) return alert('لطفاً موضوع و متن تیکت را وارد کنید'); setUpdating(true); try { await apiClient.post('/tickets', { subject: ticketSubject, message: ticketMessage, priority: ticketPriority }); alert('✅ تیکت با موفقیت ارسال شد'); setShowTicketModal(false); setTicketSubject(''); setTicketMessage(''); setTicketPriority('medium'); await loadTickets(); } catch (error) { alert(error.response?.data?.message || 'خطا در ارسال تیکت'); } finally { setUpdating(false); } };
  const handleReply = async () => { if (!replyMessage.trim()) return alert('لطفاً متن پاسخ را وارد کنید'); setUpdating(true); try { await apiClient.post(`/tickets/${selectedTicket.id}/reply`, { message: replyMessage }); alert('✅ پاسخ ارسال شد'); setReplyMessage(''); setShowReplyModal(false); setSelectedTicket(null); await loadTickets(); } catch (error) { alert(error.response?.data?.message || 'خطا در ارسال پاسخ'); } finally { setUpdating(false); } };
