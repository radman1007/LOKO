...
const LukoPodcast = () => {
  ...
  const filteredPodcasts = useMemo(() => {
    if (searchQuery) return podcastItems.filter(p => p.title.toLowerCase().includes(searchQuery.toLowerCase()) || p.category.includes(searchQuery));
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg, fontFamily: "'Shoor', 'Shoor Rounded', sans-serif", direction: 'rtl', paddingBottom: '80px' }}>
      ...
      <Header 
        ...
        rightAction={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', background: colors.iconBg, borderRadius: '40px', padding: '8px 14px', border: `1px solid ${colors.border}`, boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }}>
              <HiOutlineSearch size={16} color={colors.textSecondary} />
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="جستجو..." style={{ flex: 1, background: 'transparent', border: 'none', padding: '0 8px', fontSize: '13px', outline: 'none', color: colors.text, direction: 'rtl', width: isMobile ? '120px' : '180px' }} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: colors.xpBg, padding: '6px 12px', borderRadius: '20px' }}>
              <img src={Icon3} alt="xp" style={{ width: '14px', height: '14px' }} />
              <span style={{ fontSize: '12px', fontWeight: '700', color: colors.text }}>{userXP} XP</span>
            </div>
          </div>
        }
      />
      ...
      <div style={{ maxWidth: isDesktop ? '1200px' : '100%', margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 24px' }}>
        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap', justifyContent: 'center' }}>
          {filterButtons.map((btn) => (
            <button key={btn.id} onClick={() => toggleFilter(btn.id)} style={{ padding: isMobile ? '8px 20px' : '10px 28px', borderRadius: '30px', border: 'none', background: activeFilters.includes(btn.id) ? btn.color : '#E8E0D5', color: activeFilters.includes(btn.id) ? 'white' : colors.textSecondary, fontSize: isMobile ? '13px' : '14px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s ease' }}>{btn.title}</button>
          ))}
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
          {filteredPodcasts.map((podcast) => (
            <div key={podcast.id} onClick={() => handlePlayPodcast(podcast)} style={{ backgroundColor: colors.cardBg, borderRadius: '20px', padding: '16px', display: 'flex', gap: '16px', cursor: 'pointer', transition: 'transform 0.2s ease, box-shadow 0.2s ease', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', border: `1px solid ${colors.border}` }} onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 8px 20px rgba(0,0,0,0.12)'; }} onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(0,0,0,0.08)'; }}>
              <img src={podcast.cover} alt={podcast.title} style={{ width: '70px', height: '70px', borderRadius: '16px', objectFit: 'cover' }} />
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: '15px', fontWeight: '700', color: colors.text, marginBottom: '4px' }}>{podcast.title}</h3>
                <p style={{ fontSize: '11px', color: colors.textSecondary, marginBottom: '6px' }}>{podcast.category}</p>
              ...
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: colors.primary, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><HiOutlinePlay size={20} color="white" /></div>
            </div>
          ))}
        </div>
      ...
      {isPlayerOpen && currentPodcast && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: colors.playerBg, borderRadius: '28px 28px 0 0', padding: '20px', zIndex: 200, animation: 'slideUp 0.3s ease', boxShadow: '0 -10px 30px rgba(0,0,0,0.2)' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, right: 0, left: 0, background: colors.navBg, boxShadow: '0 -2px 20px rgba(0,0,0,0.08)', borderRadius: '28px 28px 0 0', padding: isMobile ? '10px 12px' : '12px 20px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 100 }}>
        {navItems.map((item) => { const IconComponent = item.icon; const isActive = activeTab === item.id; return (<div key={item.id} onClick={() => { setActiveTab(item.id); navigate(item.path); }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }}><IconComponent size={isMobile ? 20 : 22} color={isActive ? colors.primary : '#999'} /><span style={{ fontSize: isMobile ? '9px' : '10px', color: isActive ? colors.primary : '#999', fontWeight: isActive ? '600' : '400' }}>{item.name}</span>{isActive && <div style={{ width: '4px', height: '4px', borderRadius: '2px', background: colors.primary, marginTop: '2px' }} />}</div>); })}
      </div>

      <style>{` @keyframes slideUp { from { transform: translateY(100%); } to { transform: translateY(0); } } ::-webkit-scrollbar { display: none; } * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; } button, div[onClick] { cursor: pointer; } input { user-select: text; } `}</style>
    </div>
  );
};
