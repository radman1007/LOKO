...
const LukoHealth = () => {
  ...
  const colors = {
    ...
    text: '#455A64', cardBg: '#F1FBFA', white: '#FFFFFF',
  ...
  const gardenActions = [
    ...
    { id: 'water', icon: ``, label: 'آب', message: 'به گیاهات آب دادی! باغچه تو شاداب شد' },
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: "'Shoor', 'Shoor Rounded', sans-serif", direction: 'rtl', paddingBottom: '80px', background: `linear-gradient(180deg, ${colors.accent} 0%, ${colors.primary} 100%)` }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: isMobile ? '20px' : '24px', fontWeight: '700', color: colors.text, marginBottom: '12px', position: 'relative', zIndex: 2 }}>
            بیا باهم نفس بکشیم
          </h3>
          <p style={{ fontSize: '14px', color: colors.textSecondary, marginBottom: '40px', position: 'relative', zIndex: 2 }}>
          ...
          {isBreathing && (
            <div style={{ marginTop: '16px', position: 'relative', zIndex: 2 }}>
              <div style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: '700', color: breathPhase === 'inhale' ? colors.blue : colors.purple, marginBottom: '8px' }}>
                {breathPhase === 'inhale' ? 'نفس بگیر...' : 'نفس بده بیرون...'}
              </div>
              <div style={{ fontSize: isMobile ? '32px' : '40px', fontWeight: 'bold', color: breathPhase === 'inhale' ? colors.blue : colors.purple }}>
                {breathCount}
              </div>
              <div style={{ fontSize: '13px', color: colors.textSecondary }}>از 5</div>
            </div>
          )}
          
          {!isBreathing && (
            <div style={{ marginTop: '24px', fontSize: '14px', color: colors.blue, opacity: 0.8, position: 'relative', zIndex: 2, animation: 'pulseText 1.5s infinite' }}>
              روی روباه بزن
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h3 style={{ fontSize: isMobile ? '16px' : '18px', fontWeight: '700', color: colors.text }}>پادکست گوش بده</h3>
          <button
        ...
        <div
          ...
          onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.boxShadow = '0 15px 35px rgba(0,0,0,0.15)'; }}
          onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 25px rgba(0,0,0,0.1)'; }}
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
            <div style={{ flexShrink: 0, width: isMobile ? '70px' : '85px', height: isMobile ? '70px' : '85px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src={Icon45} alt="podcast" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
            </div>
            <div style={{ flex: 1, textAlign: 'left', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', direction: "ltr", flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
              <div>
                <h4 style={{ fontSize: isMobile ? '14px' : '16px', fontWeight: '600', color: colors.text, marginBottom: '4px' }}>
              ...
              >
                ...
                <span style={{ fontSize: isMobile ? '12px' : '13px', fontWeight: '600', color: 'white' }}>
                  گوش بده
                </span>
              </div>
            </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h3 style={{ fontSize: isMobile ? '16px' : '18px', fontWeight: '700', color: colors.text, marginBottom: '16px' }}>باغچه تو امروز سبز کن</h3>
          
          {warningMessage && (
            <div style={{ backgroundColor: '#FFECB3', borderRadius: '20px', padding: '10px 16px', marginBottom: '16px', fontSize: '13px', color: '#E65100', textAlign: 'center', animation: 'fadeInUp 0.3s ease', borderRight: '4px solid #FF9800' }}>
              {warningMessage}
            </div>
          )}
          ...
          >
            ...
            {isAnimating && (
              <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 2 }}>
                {[...Array(30)].map((_, i) => (
                  <div key={i} style={{ position: 'absolute', left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, width: '6px', height: '6px', borderRadius: '50%', backgroundColor: '#4CAF50', animation: `particleFly ${Math.random() * 1 + 0.8}s ease-out forwards` }} />
                ))}
              </div>
            )}
            {showMessage && isAnimating && (
              <div style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(10px)', borderRadius: '20px', padding: '12px 20px', fontSize: '14px', color: colors.text, textAlign: 'center', fontWeight: '600', whiteSpace: 'nowrap', zIndex: 10, animation: 'fadeInUp 0.3s ease' }}>
                {showMessage}
              </div>
            )}
            
            <div style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', minHeight: isMobile ? '380px' : '420px', padding: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '30px', padding: '16px' }}>
                {gardenActions.map((action) => {
                  ...
                  return (
                    ...
                    >
                      <img src={action.icon} alt={action.label} style={{ width: '55px', height: '55px', objectFit: 'contain' }} />
                      <span style={{ fontSize: '12px', fontWeight: '500', color: 'white', textShadow: '0 1px 2px rgba(0,0,0,0.3)', backgroundColor: 'rgba(0,0,0,0.3)', padding: '4px 12px', borderRadius: '20px', backdropFilter: 'blur(4px)' }}>
                        {action.label}
                      </span>
                    </button>
                  );
      ...
      <div style={{ position: 'fixed', bottom: 0, right: 0, left: 0, background: 'white', boxShadow: '0 -2px 20px rgba(0,0,0,0.08)', borderRadius: '28px 28px 0 0', padding: isMobile ? '10px 12px' : '12px 20px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 100 }}>
        {navItems.map((item) => {
